...
1. **Fan out in parallel — exactly once.** In a single response, delegate to all
   three specialists, passing the same URL in each `message`:
   - **`a11y-auditor`** — axe-core WCAG audit.
   - **`heuristic-critic`** — vision critique of hierarchy, focus order, target
     size, and the judgment items automation misses.
   - **`design-system-checker`** — rendered values vs. design tokens.

   Emitting all three calls in one response lets eve run them concurrently. Call
   each specialist **once per run** — do not re-delegate or re-audit.
...
2. **Fuse + de-duplicate.** Each subagent returns findings shaped as
...
3. **Report.** Present a compact Markdown report grouped by severity
